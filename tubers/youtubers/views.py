...
def youtuber_detail(request, id):
    # this method gives single object instead of all data
...
def search(request):
    tubers = Youtuber.objects.order_by('-created_date')
    # get all city name in a list form from databases
    ...
    if 'city' in request.GET:
        ...
        if city:
            tubers = tubers.filter(city__iexact=city) # exact city name we get
